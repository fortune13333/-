...
const SkeletonCard: React.FC = () => (
  <div className="bg-indigo-900 p-4 rounded-lg shadow-md animate-pulse">
    <div className="flex items-center gap-4">
      <div className="w-12 h-12 bg-indigo-800 rounded-md"></div>
      <div className="flex-1 space-y-2">
        <div className="h-4 bg-indigo-800 rounded w-3/4"></div>
        <div className="h-3 bg-indigo-800 rounded w-1/2"></div>
      </div>
    </div>
    <div className="mt-4 border-t border-indigo-800 pt-3 space-y-2">
      <div className="h-3 bg-indigo-800 rounded w-full"></div>
      <div className="h-3 bg-indigo-800 rounded w-1/2"></div>
...
const Dashboard: React.FC<DashboardProps> = ({ devices, blockchains, onSelectDevice, isLoading, onResetData, onDeleteDevice, onOpenAddDeviceModal }) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6 border-b-2 border-indigo-800 pb-2 gap-4">
        ...
        <div className="flex items-center gap-2">
            ...
            <button
              onClick={onResetData}
              className="text-sm bg-indigo-800 hover:bg-red-600/50 text-zinc-300 hover:text-red-300 font-medium py-2 px-3 rounded-md transition-colors duration-200"
            >
              重置数据
            </button>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="text-center py-20 bg-indigo-900 rounded-lg">
          ...
          {devices.map(device => {
            ...
            return (
              <div 
                ...
                className="relative bg-indigo-900 p-4 rounded-lg shadow-md cursor-pointer hover:bg-indigo-800 transition-all duration-200 group flex flex-col justify-between border border-transparent hover:border-cyan-500/50 hover:shadow-lg hover:shadow-cyan-500/10"
              >
                <button
                  onClick={(e) => handleDelete(e, device.id, device.name)}
                  className="absolute top-2 right-2 p-1.5 rounded-full bg-indigo-800/50 text-zinc-400 opacity-0 group-hover:opacity-100 hover:bg-red-500/50 hover:text-red-300 transition-all duration-200 z-10"
                ...
                <div>
                  <div className="flex items-center gap-4">
                    <div className="bg-indigo-950 p-2 rounded-md">
                ...
                <div className="mt-4 text-sm text-zinc-300 border-t border-indigo-800 pt-3 space-y-2">
                  {lastBlock ? (
                    <>
